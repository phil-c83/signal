...
function SigSetFeatures = Sig_features(s,FFT,Fe,SigSetFreqs,tol,dbg)
  ...
  SigSetFeatures.NoisePower    = sum(Sig_dsp(FFT,NoiseIdx(2:end)));
...
%    0<= phin < 2*pi
function phin=Sig_phase_normalize(phi)
  phin = mod(phi,2*pi);
endfunction

%     -pi < phiatan2 <= +pi iff  0<= phi < 2*pi
function phiatan2=Sig_phase_atan2(phi)
  ix = find(phi > pi);
  phiatan2 = phi;
  phiatan2(ix) = phiatan2(ix) - 2*pi;
endfunction

%{
 phi : arg(H(i*w)) ie band pass filter transfert function
       H(i*w) = |H(i*w)| * exp(i*w(t-tau)) = |H(i*w)| * exp(i*w*t - phi)
 tau : phi/w
%}
function tau=Sig_filter_delay(phi,w)
  tau = phi ./ w ;
endfunction
