...
# Save buffer in file
def save_buffer(Te,U1buf,I1buf,I2buf,label,cos_ui):  
    fname = '{}_{}.csv'.format(label,time.time())   
    cos_str = '#cos_ui={}'.format(cos_ui)  
    with open(fname, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(('Te','U1','I1','I2',cos_str))
        for i in range(0,len(Te)):
            row = (Te[i],U1buf[i],I1buf[i],I2buf[i])
            writer.writerow(row)
...
if __name__ == '__main__':    
    ...
    # print args
    print('devdac=%s devadc=%s fadc=%d fdac=%d fsig=%d vdac=%.1f G1=%.3f G2=%.3f Av=%.3f m=%.3f dir=%s' % 
            (args.devdac,args.devadc,fe_adc,fe_dac,fsig,vdac,G1,G2,Av,m,dname))  
    ...
    I1buf,U1buf,I2buf = nid.generate_and_sample(sig,fe_dac,fe_adc,fe_adc*(0.5-0.2),args.devdac,args.devadc) 
    Te_adc =  [t*1/fe_adc for t in range(0,len(I1buf))]
    U1rms,I1rms,P1,S1,I2rms,phi1,phi2,phi12,cos_mc,cos_ui = jm.measures_sinus(fsig,Te_adc,U1buf,I1buf,I2buf,G1,G2)
    if args.s:
       save_buffer(Te_adc,U1buf,I1buf,I2buf,"U1_I1_I2",cos_ui) 
    ...
    print('U1rms=%1.3f I1rms=%1.3f I2rms=%1.3f P1=%1.3f S1=%1.3f phi_UI=%1.3f cos_mc=%1.3f cos_ui=%1.3f' % 
            (U1rms,I1rms,I2rms,P1,S1,phi12,cos_mc,cos_ui))
